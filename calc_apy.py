...
import pandas as pd
from tool import get_token_price, round_decimal, get_token_id
        ...
        for index, row in df[table].iterrows():
            ...
            if date_row == datetime.fromtimestamp(date).date():
                ...
                if row["Total Gas Fee(ETH)"] != 0:
                    gas_to_coin = row["Total Gas Fee(ETH)"] / get_token_price(coin_id, date_str, "eth")
                
                profit += row["Total Input Amount"] - row["Total Output Amount"] - row["Total LP Fee"] - gas_to_coin
                break
...
def calc_apy_daily(cursor):
    ...
    if not existing_df.empty:
        ...
        start_time = last_date.timestamp() + 86400  # Start from the next day
        ...
        start_time = 1724803200  # Example start time
        last_eth_capital = 0
        last_usdc_capital = 0
        last_btc_capital = 0
        last_dai_capital = 0
    ...
    while start_time < end_time:
        ...
        if eth_profit or eth_capital:
            weth_price = Decimal(get_token_price("weth", date_str))
        if usdc_profit or usdc_capital:
            usdc_price = Decimal(get_token_price("usd-coin", date_str))
        if btc_profit or btc_capital:
            wbtc_price = Decimal(get_token_price("wrapped-bitcoin", date_str))
        if dai_profit or dai_capital:
            dai_price = Decimal(get_token_price("dai", date_str))
        if usdt_profit or usdt_capital:
            usdt_price = Decimal(get_token_price("tether", date_str))
