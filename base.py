...
def insert_return_data(contract, cursor, block=17987144):
    ...
    for event in event_list:
      ...
      if address_to_check in refund_addresses:
         ...
         for index in indices:
            cursor.execute('''
                  INSERT INTO Return (
                     tx_hash, output_token, output_amount, aim_chain, block
                  ) VALUES (?, ?, ?, ?, ?)
            ''', (
                  event['transactionHash'].hex(), event['args']['l2TokenAddress'], event['args']['refundAmounts'][index], 'base', int(event['blockNumber'])
            ))
    return
...
def main():
    ...
    with open('spoke_abi.json', 'r', encoding='utf-8') as file:
         abi = json.load(file)
    web3 = Web3(Web3.HTTPProvider('https://base-mainnet.g.alchemy.com/v2/pMl8wAjVvCRAzgqtijiX_4Wn60sat_m9'))
    ...
    # Get the last block number from LastBlock table
    cursor.execute('SELECT last_block FROM LastBlock WHERE chain_name = ?', ('base',))
    ...
    url = (
        f'https://api.basescan.org/api?module=account&action=txlist&address=0x84A36d2C3d2078c560Ff7b62815138a16671b549&startblock={last_block}&endblock=99999999&sort=asc&apikey=66BWR5G2BKHJUKNQQ3D65JM2M4VIGRKJEB'
    )
    ...
    for tx in data['result']:
      if tx['methodId'] == '0x2e378115':
         ...
         cursor.execute('''
               INSERT INTO Fill (
                  ...
                  aim_chain, timestamp, block, origin_chain,
      ...
      cursor.execute('''
         ...
         ON CONFLICT(chain_name) DO UPDATE SET last_block=excluded.last_block
        ''', (
