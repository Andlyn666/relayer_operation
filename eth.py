...
def get_latest_eth_block(cursor):
    # Get the last block number from Variable table
    cursor.execute("SELECT value FROM Variable WHERE name = ?", ("eth_block",))
    result = cursor.fetchone()
    last_block = int(result[0]) + 1 if result else 20299389
    return last_block
...
def update_eth():
    ...
    last_block = get_latest_eth_block(cursor)

    #  print(last_block)
    eth_key = os.getenv("ETH_KEY")
    # Define the URL with query parameters
    url = f"https://api.etherscan.io/api?module=account&action=txlist&address=0x84A36d2C3d2078c560Ff7b62815138a16671b549&startblock={last_block}&endblock=999999999&sort=asc&apikey={eth_key}"
